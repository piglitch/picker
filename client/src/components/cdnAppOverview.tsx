...
const CdnAppOverView = () => {
  ...
  const fetcher = async() => {  
    ...
    const data = await fetch(`http://localhost:3000/api/${user?.id}/user-apps/`);
  ...
  async function fetchFilesS3() {
    try {
      const response = await fetch(`http://localhost:3000/api/${user?.id}/file-details`);
        ...
        <div className='w-5/6 px-6'>
          ...
          <div className='bg-slate-100 mt-4 rounded-md shadow-md h-[500px] w-lg'>
            <Gauge 
              ...
              value={appsize*100/1024} valueMin={0} valueMax={100} 
            />
